  ...
  // ── 相簿模式 ────────────────────────────────────────────────────────────────
  // 当 URL 带 ?album=<slug> 时，从 /api/albums/:slug 取照片列表，
  // 用同一套极光墙 + 灯箱渲染，隐藏日期导航，顶部显示相簿标题
  function loadAlbumMode(slug, pw) {
    const content = document.getElementById('content');
    const subtitle = document.getElementById('subtitle');
    const FADE_MS = 220;

    // 相簿模式：隐藏不必要的日期控件
    const elHide = ['lunarToggle','yearToggle','dateToggle','yearMenu','otdBlock',
                     'recapLink','lovedLink','calendarModeToggle'];
    elHide.forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.style.display = 'none';
    });
    document.querySelectorAll('.sunlight-switch').forEach((el) => el.style.display = 'none');
    // 把相册列表回退链接插进 header controls
    const controls = document.querySelector('.header-controls');
    if (controls) {
      const backLink = document.createElement('a');
      backLink.className = 'date-toggle';
      backLink.href = '/albums';
      backLink.title = '返回相簿列表';
      backLink.innerHTML = '<svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>';
      controls.prepend(backLink);
    }

    const seq = ++_memSeq;
    const headers = pw ? { 'x-album-password': pw } : {};
    ...
    fetch('/api/albums/' + encodeURIComponent(slug), { headers }).then((r) => {
      if (r.status === 401) return r.json().then((d) => { throw Object.assign(new Error('auth'), { code: 'password_required' }); });
      ...
      subtitle.textContent = data.description || (data.photos.length + ' 张照片');

      if (!data.photos.length) {
        fadeOut().then(() => {
          if (seq !== _memSeq) return;
          content.innerHTML = '<div class="empty">这个相簿还没有照片</div>';
          fadeIn();
        });
        return;
      }

      // 按年分组，与 loadMemories 格式对齐
      ...
      cellCenters.clear();
      allPhotos = [];
      yearGroups.forEach((y) => y.photos.forEach((p) => allPhotos.push({ ...p, year: y.year })));
      ...
      const FEATURED_LIMIT = 10;
      function pickFeatured(photos, limit) {
        if (photos.length <= limit) return null;
        const featured = new Set();
        photos.forEach((p, i) => { if ((p.type === 'video' || p.type === 'live') && featured.size < limit) featured.add(i); });
        const scoredIdx = photos.map((p, i) => ({ i, score: p.score })).filter(({ i, score }) => !featured.has(i) && typeof score === 'number').sort((a, b) => b.score - a.score);
        for (const { i } of scoredIdx) { if (featured.size >= limit) break; featured.add(i); }
        const remainingIdx = photos.map((_, i) => i).filter((i) => !featured.has(i));
        const need = limit - featured.size;
        if (need > 0 && remainingIdx.length > 0) {
          const step = remainingIdx.length / need;
          for (let k = 0; k < need; k++) featured.add(remainingIdx[Math.min(remainingIdx.length - 1, Math.floor(k * step))]);
        }
        return featured;
      }
      ...
      const renderYearBlock = (y) => {
        ...
        const cells = y.photos.map((p, pi) => {
          ...
          const thumbW = Math.round((isMobileLayout ? mobileRenderSize : size) * dpr);
          const thumbSrc = p.url.replace('/img/', '/thumb/') + '?w=' + thumbW + '&q=75&fit=scale-down';
          const reactionCnt = (_roomReactions[p.key] > 0 ? _roomReactions[p.key] : '');
          if (p.type === 'video') {
            return `<div class="cell${extraClass}" style="${style}" onclick="openLightbox(${flatIndex}, false)"><div class="frame-inner"><video data-src="${escAttr(p.url)}#t=0.5" muted loop playsinline preload="metadata" onloadeddata="this.classList.add('loaded')" onmouseenter="this.play().catch(()=>{})" onmouseleave="this.pause();this.currentTime=0.5"></video></div><span class="play-badge">▶ 视频</span><span class="frame-year">${y.year}</span><button class="react-btn" data-key="${escAttr(p.key)}" onclick="event.stopPropagation();doReact(this)">❤️<span class="react-cnt">${reactionCnt}</span></button></div>`;
          }
          if (p.type === 'live') {
            return `<div class="cell${extraClass}" style="${style}" onclick="openLightbox(${flatIndex}, false)"><div class="frame-inner live-photo-cell" onmouseenter="this.classList.add('playing');const v=this.querySelector('video');v.loop=true;v.currentTime=0;v.play().catch(()=>{})" onmouseleave="this.classList.remove('playing');this.querySelector('video').pause()" ontouchstart="livePhotoTouchStart(this,event)" ontouchend="livePhotoTouchEnd(this,event)" ontouchcancel="livePhotoTouchEnd(this,event)"><img src="${escAttr(thumbSrc)}" data-src="${escAttr(p.url)}" loading="lazy" decoding="async" onload="this.classList.add('loaded')" onerror="this.onerror=null;scheduleImageRetry(this,this.src,this.dataset.src)" /><video src="${p.videoUrl}" loop muted playsinline preload="none" class="cell-live-video"></video></div><span class="frame-year">${y.year}</span><button class="react-btn" data-key="${escAttr(p.key)}" onclick="event.stopPropagation();doReact(this)">❤️<span class="react-cnt">${reactionCnt}</span></button></div>`;
          }
        ...
        const showMoreBtn = extraCount > 0
          ? `<button class="show-more-btn" data-total="${y.photos.length}" onclick="toggleShowMore(this)">展开查看全部 ${y.photos.length} 张 ›</button>`
          : '';
        return `<div class="year-block" id="year-${y.year}"><div class="year-title">${y.year} 年 <span class="count">（${y.photos.length} 份）</span></div><div class="grid">${cells}</div>${showMoreBtn}</div>`;
      };

      const apply = () => {
        content.innerHTML = yearGroups.map((y) => renderYearBlock(y)).join('');
        content.querySelectorAll('.cell').forEach((cell) => cellObserver.observe(cell));
        content.querySelectorAll('.cell video[data-src]').forEach((v) => videoLazyObserver.observe(v));
        fadeIn();
      };
      ...
      if (err.code === 'password_required') {
        subtitle.textContent = '私密相簿';
        content.innerHTML = '<div class="empty">此相簿需要密码，请从<a href="/albums" style="color:#3a82f7;text-decoration:none">相簿列表</a>输入密码后进入</div>';
        fadeOut().then(() => { if (seq !== _memSeq) return; content.style.opacity = '1'; });
        return;
      }
      subtitle.textContent = '加载失败';
      content.innerHTML = '<div class="empty">相簿加载失败，请检查网络后重试</div>';
    });
  }

  const _albumSlug = params.get('album');
  const _albumPw = params.get('pw');
  if (_albumSlug) {
    loadAlbumMode(_albumSlug, _albumPw);
  } else {
    loadMemories(month, day);
  }